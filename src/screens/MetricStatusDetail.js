...
function HealthBar({
  ...
  return (
    <View>
      <View
        style={{
          marginTop: enabledMargin ? 40 : 0,
          height: 24,
          width,
          flexDirection: 'row',
          justifyContent: 'space-evenly',
        }}>
        {['green', 'green', 'yellow', 'yellow', 'red', 'red'].map(
          (color, index) => (
            <View
              key={index}
              style={{
                height: '100%',
                ...tailwind(`w-2 bg-${color}-400 rounded-full`),
              }}></View>
          ),
        )}
      </View>
...
function MetricStatusDetail({route}) {
  ...
  return (
    <SafeAreaView style={{backgroundColor: '#374151', height: '100%'}}>
      <Text style={tailwind('text-white font-bold text-3xl ml-6 mt-6')}>
        {type}
      </Text>
      <Text style={tailwind('text-white text-base text-gray-400 ml-6 mt-2')}>
        {viewingTank}
      </Text>
      <View
        style={{
